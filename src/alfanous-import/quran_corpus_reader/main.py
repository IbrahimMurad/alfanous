                ...
                for i in range(len(part)):
                    ...
                    if tag[-1] == ":":
                        ...
                        if tag in PGN:
                            ...
                            if nexttag == "PCPL":
                                partDict[
                                    reverse_class(DERIVclass)[tag + " PCPL"][0]
                                ] = DERIV[tag + " PCPL"][1]
                                i += 1
                        elif tag in VERB:
                            ...
                            partDict[arabize(reverse_class(NOMclass)[tag][0])] = NOM[
                                tag
                            ][0]
        ...
        for chapter in self.corpus.findall(".//chapter"):
            for verse in chapter.findall("verse"):
                for word in verse.findall("word"):
                    D[word.attrib["token"]] = API.MorphologyParser.parse(
        ...
        for chapter in self.corpus.findall(".//chapter"):
            for verse in chapter.findall("verse"):
                for word in verse.findall("word"):
                    ...
                    res["morphology"] = API.MorphologyParser.parse(
                        word.attrib["morphology"]
                    )
                    yield res

...
from alfanous.Support.whoosh.analysis import LowercaseFilter, RegexTokenizer
...
class QuranicCorpusImporter:
    def __init__(
        self, QC_PATH="../../../store/quranic-corpus-morpology.xml", DB="main.db"
    ):
        import sqlite3
        ...
        for iteration in A.all_words_generator():
            ...
            QUERY = (
                ...
                % {
                    ...
                    "prefixes": ";".join([
                        prefix["arabictoken"] for prefix in glob["prefixes"]
                    ]).encode("utf-8"),
                    "suffixes": ";".join([
                        suffix["arabictoken"] for suffix in glob["suffixes"]
                    ]).encode("utf-8"),
                    "type": IFEXIST(d, "type"),
                    "pos": IFEXIST(d, "pos"),
                    ...
                    "spelled": QASF_spelled.normalize_all(iteration["word"]).encode(
                        "utf-8"
                    ),
...
class ZekrModelsImporter:
    ...
    def test_existence(self, id):
        ...
        if l == 6236:
            ...
            print(
                "\nwarning: the translation (id= %(id)s ) has incomplete documents nb= %(l)d"
                % {"id": id, "l": l}
            )
            return l

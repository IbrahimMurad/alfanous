...
class Transformer:
    ...
    def build_schema(self, tablename):
        ...
        resSchema = None
        # Provide namespace for exec with all required classes
        namespace = {
            "Schema": Schema,
            "NUMERIC": NUMERIC,
            "TEXT": TEXT,
            ...
            "QDiacAnalyzer": QDiacAnalyzer,
            "QUthmaniDiacAnalyzer": QUthmaniDiacAnalyzer,
            "QUthmaniAnalyzer": QUthmaniAnalyzer,
            "APermissibleAnalyzer": APermissibleAnalyzer,
        }
        exec("resSchema=" + Schema_raw, namespace)
        resSchema = namespace["resSchema"]
    ...
    def transfer(self, ix, tablename="aya"):
        ...
        for field in schema.field_names():
            seq.append(field)
        query = (
            "select name,search_name from field where search_name IN ('"
            + "','".join(seq)
            + "')"
        )
        cur.execute(query)
        ...
        for line in Data:
            ...
            for field in schema.field_names():
                ...
                if isinstance(v, str):
                    ...
                    write_cmd += f + "=" + str(v) + ","  # must change 1 to 0001
                else:
                    pass
                i += 1
    ...
    def __unlock_docindex(self, ix):
        # Use the passed index instead of trying to open it
        # ix = self.__storage.open_index()
        # return ix.unlock()
        pass
...
if __name__ == "__main__":
    T = Transformer(
        ixpath="../../indexes/main/",
        dypypath="../alfanous/dynamic_resources/",

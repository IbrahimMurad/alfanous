...
def SCAN_SUPERJOKERS(query):
    ...
    super_joker = (
        True
        if (len(filtred_query) < 3 and "*" in query)
...
class Raw:
    ...
    def _search_aya(self, flags):
        ...
        range = (
            self._defaults["minrange"] if range < self._defaults["minrange"] else range
        )
        range = (
            self._defaults["maxrange"] if range > self._defaults["maxrange"] else range
        )
        ...
        V = QArabicSymbolsFilter(
            shaping=False, tashkil=not vocalized, spellerrors=False, hamza=False
        ).normalize_all
                ...
                if term[0] == "aya" or term[0] == "aya_":
                    ...
                    if term[0] == "aya_":
                        ...
                        annotation_word_query += (
                            " OR normalized:%s " % STANDARD2UTHMANI(term[1])
                        )
                    ...
                    if word_derivations:
                        lemma = LOCATE(
                            derivedict["word_"], derivedict["lemma"], term[1]
                        )
                        if lemma:  # if different of none
                            derivations = FILTER_DOUBLES(
                                FIND(derivedict["lemma"], derivedict["word_"], lemma)
                            )
                        else:
                            derivations = []
                    ...
                    words_output["individual"][cpt] = {
                        ...
                        "romanization": transliterate(
                            romanization, term[1], ignore="", reverse=True
                        )
                        if romanization in self.DOMAINS["romanization"]
                        else None,
                        "nb_matches": term[2],
                        "nb_ayas": term[3],
        ...
        if annotation_aya or (annotation_word and word_info):
            annotation_word_query = (
                annotation_word_query if annotation_word and word_info else "()"
            )
            ...
            for annot in annot_res:
                if annotation_word and word_info:
                    if annot["normalized"] in terms_uthmani:
                        if annotations_by_word.has_key(annot["normalized"]):
                            if annotations_by_word[annot["normalized"]].has_key(
                                annot["word"]
                            ):
                ...
                if annotation_aya:
                    if annotations_by_position.has_key((
                        ...
                        annotations_by_position[(annot["sura_id"], annot["aya_id"])][
                            annot["word_id"]
                        ] = annot
                    else:
                        annotations_by_position[(annot["sura_id"], annot["aya_id"])] = {
        ...
        for r in reslist:
            ...
            output["ayas"][cpt] = {
                ...
                "aya": {
                    ...
                    "text_no_highlight": r["aya"]
                    if script == "standard"
                    else r["uth_"],
                    "translation": trad_text[r["gid"]]
                    if (
                        translation != "None"
                        and translation
                        and trad_text.has_key(r["gid"])
                    )
                    else None,
                    "recitation": None
                ...
                "annotations": {}
                if not annotation_aya
                or not annotations_by_position.has_key((r["sura_id"], r["aya_id"]))
    ...
    def _search_translation(self, flags):
        ...
        SE = self.TSE
        res, termz, searcher = SE.search_all(
            query, self._defaults["results_limit"]["translation"]
        )
        terms = [term[1] for term in list(termz)[: self._defaults["maxkeywords"]]]
        # pagination
        offset = 1 if offset < 1 else offset
        ...
        interval_end = offset + range - 1
            ...
            interval_end
            if interval_end < len(res)
            else len(res)
            if len(res) < self._defaults["results_limit"]["translation"]
        ...
        limited_total = (
            total
            if total < self._defaults["results_limit"]["translation"]
            else self._defaults["results_limit"]["translation"]
        )
        ...
        for r in reslist:
            ...
            output["words"][cpt] = {
                ...
                if not aya
                    ...
                    "text": SE.highlight(
                        aya_info[r["sura_id"]][r["aya_id"]]["uth_"],
                        [r["word"]],
                        highlight,
                        False,
                    ),
                    ...
                    "sura_arabic_name": aya_info[r["sura_id"]][r["aya_id"]][
                        "sura_arabic"
                    ],
                },
            }

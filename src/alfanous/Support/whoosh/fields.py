...
from alfanous.Support.whoosh.analysis import (
    IDAnalyzer,
    KeywordAnalyzer,
    NgramAnalyzer,
    RegexAnalyzer,
...
from alfanous.Support.whoosh.formats import Existence, Format, Frequency, Positions
...
class FieldType(object):
    ...
    * unique (boolean): whether this field's value is unique to each document.
    ...
    indexed = True
    __inittypes__ = dict(
        format=Format, vector=Format, scorable=bool, stored=bool, unique=bool
    )
    ...
    def index(self, value, **kwargs):
        ...
        if not isinstance(value, str):
            raise ValueError("%r is not str" % value)
...
class DATETIME(FieldType):
    ...
    def index(self, dt):
        if not isinstance(dt, datetime.datetime):
            raise ValueError(
                "Value of DATETIME field must be a datetime object: %r" % dt
            )
...
class TEXT(FieldType):
    ...
    __inittypes__ = dict(
        analyzer=object, phrase=bool, vector=Format, stored=bool, field_boost=float
    )

    def __init__(
        self, analyzer=None, phrase=True, vector=None, stored=False, field_boost=1.0
    ):
        """
...
class NGRAM(FieldType):
    ...
    def __init__(self, minsize=2, maxsize=4, stored=False, field_boost=1.0):
        ...
        self.format = Frequency(
            analyzer=NgramAnalyzer(minsize, maxsize), field_boost=field_boost
        )
        self.scorable = True
        self.stored = stored
...
class Schema(object):
    ...
    def __getitem__(self, id):
        ...
        fieldnum = int(id)
        if fieldnum < 0 or fieldnum >= len(self._by_number):
            raise IndexError("Field number %d is out of range (0-%d)" % (fieldnum, len(self._by_number) - 1))
        return self._by_number[fieldnum]

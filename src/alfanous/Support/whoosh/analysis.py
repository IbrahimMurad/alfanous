...
class Token(object):
    ...
    def __repr__(self):
        parms = ", ".join(
            "%s=%r" % (name, value) for name, value in self.__dict__.iteritems()
        )
...
class Composable(object):
    ...
    def __repr__(self):
        ...
        if self.__dict__:
            attrs = ", ".join(
                "%s=%r" % (key, value) for key, value in self.__dict__.iteritems()
            )
...
class IDTokenizer(Tokenizer):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        assert isinstance(value, str), "%r is not str" % value
...
class IntraWordFilter(Filter):
    ...
    * `Super-Duper-XL500-42-AutoCoder!` -> `0`:`Super`, `1`:`Duper`, `2`:`XL`,
    ...
    __inittypes__ = dict(
        delims=str, splitwords=bool, splitnums=bool, mergewords=bool, mergenums=bool
    )
...
def StandardAnalyzer(
    expression=r"\w+(\.?\w+)*", stoplist=STOP_WORDS, minsize=2, gaps=False
):
...
StandardAnalyzer.__inittypes__ = dict(
    expression=str, gaps=bool, stoplist=list, minsize=int
)
...
StemmingAnalyzer.__inittypes__ = dict(
    expression=str, gaps=bool, stoplist=list, minsize=int
)
...
FancyAnalyzer.__inittypes__ = dict(
    expression=str, gaps=bool, stoplist=list, minsize=int
)

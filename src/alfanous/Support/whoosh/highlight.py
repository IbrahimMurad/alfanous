...
class HtmlFormatter(object):
    ...
    def __call__(self, text, fragments):
        seen = self.seen
        return self.between.join(
            self._format_fragment(text, fragment, seen) for fragment in fragments
        )
...
class GenshiFormatter(object):
    ...
    def __init__(self, qname="strong", between="..."):
        ...
        from genshi.core import END, START, TEXT, Attrs, Stream  # @UnresolvedImport
...
def top_fragments(
    text, terms, analyzer, fragmenter, top=3, scorer=BasicFragmentScorer, minscore=1
):

...
class SegmentReader(IndexReader):
    def __init__(self, storage, segment, schema):
        ...
        self._fieldnum_to_scorable_pos = dict(
            (fnum, i) for i, fnum in enumerate(self._scorable_fields)
        )

        self.termtable = open_terms(storage, segment)
        self.postfile = None
        self.docstable = open_storedfields(
            storage, segment, schema.stored_field_names()
        )
        self.doclengths = None
        if self._scorable_fields:
            self.doclengths = open_doclengths(
                storage, segment, len(self._scorable_fields)
            )
    ...
    def _open_vectors(self):
        if not self.vectortable:
            ...
            self.vpostfile = storage.open_file(
                segment.vectorposts_filename, mapped=False
            )
    ...
    def postings(self, fieldid, text, exclude_docs=frozenset()):
        ...
        if not self.postfile:
            self.postfile = self.storage.open_file(
                self.segment.posts_filename, mapped=False
            )

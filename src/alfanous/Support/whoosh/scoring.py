...
class Weighting(object):
    ...
    def fl_over_avfl(self, ixreader, docnum, fieldnum):
        ...
        """
        return ixreader.doc_field_length(docnum, fieldnum) / self.avg_field_length(
            ixreader, fieldnum
        )
...
class BM25F(Weighting):
    ...
    def score(self, searcher, fieldnum, text, docnum, weight, QTF=1):
        ...
        l = ixreader.doc_field_length(docnum, fieldnum)  # noqa: E741
    ...
    def score(self, searcher, fieldnum, text, docnum, weight, QTF=1):
        ...
        return (
            ...
            * (
                ...
                + (weight + 1.0) * (+log(post * invprior, 2))
                + 0.5 * log(post / prior, 2)
            )
        )
...
class DLH13(Weighting):
    ...
    def score(self, searcher, fieldnum, text, docnum, weight, QTF=1):
        ...
        return (
            QTF
            * (
                weight * log((weight * avl / dl) * (dc / tc), 2)
                + 0.5 * log(2.0 * pi * weight * (1.0 - f))
            )
            / (weight + k)
        )
...
class Hiemstra_LM(Weighting):
    ...
    def score(self, searcher, fieldnum, text, docnum, weight, QTF=1):
        ...
        dl = ixreader.doc_field_length(docnum, fieldnum)
        return log(
            1 + (c * weight * ixreader.field_length(fieldnum)) / ((1 - c) * tc * dl)
        )
...
class InL2(Weighting):
    ...
    def score(self, searcher, fieldnum, text, docnum, weight, QTF=1):
        ...
        dl = ixreader.doc_field_length(docnum, fieldnum)
        TF = weight * log(
            1.0 + (self.c * self.avg_field_length(ixreader, fieldnum)) / dl
        )
        norm = 1.0 / (TF + 1.0)
...
class MultiFieldSorter(FieldSorter):
    ...
    def order(self, searcher, docnums, reverse=False):
        ...
        return sorted(
            docnums, key=lambda x: tuple(c[x] for c in caches), reverse=reverse
        )

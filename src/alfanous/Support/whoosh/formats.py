...
class Frequency(Format):
    ...
    def word_values(self, value, **kwargs):
        ...
        encode = self.encode
        return ((w, freq, encode(freq)) for w, freq in seen.items())
...
class DocBoosts(Frequency):
    ...
    def word_values(self, value, doc_boost=1.0, **kwargs):
        ...
        encode = self.encode
        return ((w, freq, encode((freq, doc_boost))) for w, freq in seen.items())
...
class Positions(Format):
    ...
    def word_values(self, value, start_pos=0, **kwargs):
        ...
        for t in unstopped(
            self.analyzer(value, positions=True, start_pos=start_pos, **kwargs)
        ):
            seen[t.text].append(start_pos + t.pos)

        encode = self.encode
        return ((w, len(poslist), encode(poslist)) for w, poslist in seen.items())
    ...
    def encode(self, positions):
        ...
        for pos in positions:
            codes.append(str(varint(pos - base)))
            base = pos
        return str(pack("!I", len(positions))) + "".join(codes)
...
class Characters(Positions):
    ...
    def word_values(self, value, start_pos=0, start_char=0, **kwargs):
        ...
        encode = self.encode
        return ((w, len(ls), encode(ls)) for w, ls in seen.items())
...
class PositionBoosts(Positions):
    ...
    def word_values(self, value, start_pos=0, **kwargs):
        ...
        for t in unstopped(
            self.analyzer(
                value, positions=True, boosts=True, start_pos=start_pos, **kwargs
            )
        ):
            pos = t.pos
            boost = t.boost
...
class CharacterBoosts(Characters):
    ...
    def decode_characters(self, valuestring):
        return [
            ...
            for pos, startchar, endchar, boost in self.decode_character_boosts(
                valuestring
            )
        ]
